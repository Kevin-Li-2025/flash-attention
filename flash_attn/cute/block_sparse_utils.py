...
def produce_block_sparse_q_loads_bwd_sm100(
    ...
    q_subtile_factor: cutlass.Constexpr = 1,
    m_block_max: int = 0,
    # 2CTA schedule knobs and extra state
    use_2cta_instrs: cutlass.Constexpr = False,
    tile_hdim: cutlass.Constexpr = 0,
    producer_state_Q_Qt=None,
    producer_state_O_Ot=None,
    producer_state_LSE=None,
    producer_state_dPsum=None,
    ...
    """Produce SM100 backward block-sparse Q/dO loads for 1CTA and 2CTA schedules."""
    (
        curr_q_cnt,
        curr_q_idx,
        curr_full_cnt,
    ...
    if loop_count > Int32(0):
        if const_expr(not use_2cta_instrs):
            ...
            if const_expr(tile_hdim == 192):
                ...
                assert producer_state_Q_Qt is not None and producer_state_O_Ot is not None
                assert producer_state_LSE is not None and producer_state_dPsum is not None
                pipeline_Q.producer_acquire(
                    producer_state_Q_Qt,
                ...
                for iter_idx in cutlass.range(Int32(1), loop_count, unroll=1):
                    ...
                    if const_expr(should_load_dO):
                        pipeline_dO.producer_acquire(
                            producer_state_dO_dPsum,
                            extra_tx_count=tma_copy_bytes_dO
                            if const_expr(load_dOt is not None)
                            else 0,
                        )
